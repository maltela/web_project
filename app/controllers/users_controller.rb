class UsersController < ApplicationController
  ...
  # GET /users/1.json
  def login
    @user = User.find_by_sql(['select salt_masterkey from users Where identity like ?;', params[:identity]])
    if (@user)
      respond_with(:salt_masterkey => @user.first.salt_masterkey, :privkey_user_enc => @user.first.privkey_user_enc, :pubkey_user => @user.first.pubkey_user, status: 100)
    else
